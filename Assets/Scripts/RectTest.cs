...
public class RectTest : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        Vector2 CenterOfMass1 = CellPos1 + new Vector2(MassRect1.x + ExtentOfMass1.x/2, MassRect1.y + ExtentOfMass1.y/2);
        
        Gizmos.color = Color.Lerp(Color.gray, Color.yellow, 0.66f);
        Gizmos.DrawWireCube( new Vector3(CenterOfMass1.x,0.0f,CenterOfMass1.y), new Vector3(ExtentOfMass1.x,0,ExtentOfMass1.y) );
        Gizmos.color = Color.Lerp(Color.gray, Color.red, 0.33f);
        Gizmos.DrawLine(new Vector3(CellPos1.x, 0, CellPos1.y),new Vector3(CellPos1.x + Vel1.x, 0, CellPos1.y + Vel1.y));
        ...
        Vector2 CenterOfMass2 = CellPos2 + new Vector2(MassRect2.x + ExtentOfMass2.x/2, MassRect2.y + ExtentOfMass2.y/2);
        
        Gizmos.color = Color.Lerp(Color.gray, Color.green, 0.66f);
        Gizmos.DrawWireCube( new Vector3(CenterOfMass2.x,0.0f,CenterOfMass2.y), new Vector3(ExtentOfMass2.x,0,ExtentOfMass2.y) );
        Gizmos.color = Color.Lerp(Color.gray, Color.red, 0.33f);
        ...
        Gizmos.DrawWireCube( new Vector3(CenterOfMass1.x+MassRectOffset1.x,0.0f,CenterOfMass1.y+MassRectOffset1.y), new Vector3(ExtentOfMass1.x,0,ExtentOfMass1.y) );
        
        Vector2 MassRectOffset2 = Vel2 * DeltaTime;
        
        Gizmos.color = Color.Lerp(Color.gray, Color.green, 0.33f);
        Gizmos.DrawWireCube( new Vector3(CenterOfMass2.x+MassRectOffset2.x,0.0f,CenterOfMass2.y+MassRectOffset2.y), new Vector3(ExtentOfMass2.x,0,ExtentOfMass2.y) );
        
        // intersect <mass rect with offset #1> and <cell #1>
        
        Vector4 aabbCell1 = new Vector4( CellPos1.x - CellSize/2, CellPos1.y - CellSize/2, CellPos1.x + CellSize/2, CellPos1.y + CellSize/2 );
